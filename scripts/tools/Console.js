...
module.exports=()=>{
    ...
    let event=events.on("log",str=>{
        log(str,str.substring(str.indexOf("[]")+2));
    })
        ...
        t.button(Core.atlas.drawable("aimclient-upload"),Stys.di,run(()=>{
            new FileSelector(window,{
                ...
                onSelect:file=>{
                    ...
                    Manager.console=consoleObj;
                    Vars.mods.scripts.runConsole("var Console=require('Manager').console")
                    let result=Vars.mods.scripts.runConsole(str)
                    log("[lightgray]<[] "+result,result);
                }
            })
        })).size(32,32).get().resizeImage(16)
