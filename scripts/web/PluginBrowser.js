...
module.exports=(type,data)=>{
    ...
    window.setTitle("@pluginBrowser");

    /**
     * [search                      ][Reload]
     * [tags                                ]
     * ======================================
     * id name author              [Download]
     * desc                        
     * [tag][tag]
     * ---------------------500px------------
     * ===[usid                  ][Upload]===
    ...
    let body=new Table();
    let source=Manager.sources.plugins.get(Core.settings.get("ac-plugin-source",Manager.sources.plugins.getDefaultSource().toString()))
    if(source==undefined) source=Manager.sources.plugins.getDefaultSource()
    let draw;
    let plugins=[];
    let flush=()=>{
        ...
        ta.push(t.table(cons(t=>{
            sourceSelect=Selectors.objectSelectButton(window,t,{
                onSelect(o){
                    if(o!=null){
                        source=o
                        Core.settings.put("ac-plugin-source",o.toString())
                        flush()
                    }
                },
                datas:Manager.sources.plugins.sources
            })
    ...
    let pane=body.pane(cons(t=>{
        draw=(w)=>{
            ...
            let id=0;
            let ly=0
            amount=source.search(plugins,search.get().getText(),currTag).length
            let p=+page.get().getText()
            for(let plugin of source.search(plugins,search.get().getText(),currTag).slice(p*30,p*30+30)){
                ...
                t.table(Stys.button,cons(t=>{
                    ...
                    t.row()
                    t.label(prov(()=>ma.pluginName)).size(ww,32).get().setWrap(true)
                    t.row()
                    t.label(prov(()=>Core.bundle.get("Mauthor")+ma.author)).size(ww,32).get().setWrap(true)
                    t.row()
                    t.table(cons(t=>{
                        let wi=ww;
                        if(plugin.tags==null) plugin.tags=[]
                        let str=""
                        for(let tag of plugin.tags){
                        str+="["+source.tags[tag]+"]"
                            wi-=wi/plugin.tags.length;
                        }
