...
function UnitSelector(window,options){
    if(!window) throw new java.lang.NullPointerException("window is null");
    if(!options) options={};
    let opt=Object.assign({},defaultOptions,options);
    let lastTitle=window.title;
    window.setTitle((lastTitle.startsWith("@")?Core.bundle.get(lastTitle.substring(1)):lastTitle)+" - "+Core.bundle.get("selectUnit"));
    ...
    Vars.content.units().each(u=>{
        let modeName=u.minfo.mod?u.minfo.mod.name:"mindustry";
        if(!units[modeName]){
            units[modeName]=[]
            dspN[modeName]=u.minfo.mod?u.minfo.mod.meta.displayName:"Mindustry";
        }
        units[modeName].push(u);
    })
    let pane=body.pane(cons(t=>{
        draw=(wi)=>{
            t.clear()
            for(let mod in units){
                let m=mod;
                t.table(cons(t=>{
                    t.table(cons(t=>{
                    ...
                    let added=0;
                    t.table(cons(t=>{
                        for(let u of units[m]){
                            if(!opt.filter(u)) continue;
                            let uu=u;
                            let b=t.button(Core.atlas.drawable("aimclient-fill"),Stys.di,run(()=>{
                                e.get().setText((uu.minfo.mod?(uu.minfo.mod.name+"-"):"")+uu.name);
                                unit=uu;
                                im.clear();
                                im.image(uu.uiIcon).size(32,32);
                            })).size(32,32).get();
                            b.replaceImage(new Image(u.uiIcon));
                            b.resizeImage(32);
                            i++;
                            added++;
                            if(i>=w){
    ...
    e=statusBar.field("",Stys.textField,cons(a=>{try{
        unit=Vars.content.getByName(ContentType.unit,a)
        im.clear();
        im.image(unit.uiIcon).size(32,32);
    }catch(_){}})).size(0,0);
    e.get().setValidator(new TextField.TextFieldValidator(){
        valid(text){
            try{return Vars.content.getByName(ContentType.unit,text)!=null;}catch(e){
                return false;
            }
        }
    });
